...
router.post("/login/verify-otp", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "OTP verified successfully", user, jwtToken });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
...
router.post(
  ...
  authorize("admin"),
  async (req, res) => {
    const { currencyName, currencySymbol, minimumStake, currencyImage } =
      req.body;
    console.log("req.body", req.body);
    // ithu token ah coin ah nu api than kandupidikanum
    // curreny active and deactive status vaikanum and admin than status ah active pannanum
    //admin alltype ah table mela vaikanum
    //data illa na no data found nu admin ku vaikanum
    // config edit la duration um change pannanum
    //finally metamask ah include pannanum
    try {
    ...
    try {
      ...
      if (final === true && type === "flexible") {
        return res.status(400).json({
          message: `Already flexible type added in
...
router.get(
  "/getConfig/:currencySymbol",
  authenticate,
  authorize("admin", "user"),
  async (req, res) => {
    const { currencySymbol } = req.params;
    const config = (
      await Config.find({ currencySymbol }).sort({ plans: 1 })
    ).reverse();
...
router.post("/stake", authenticate, authorize("user"), async (req, res) => {
  const { userId, walletId, amount, duration, apr, stakeType, network } =
    req.body;
  console.log("req.body", req.body);
  ...
  try {
    ...
    if (wallet.amount - amount < 30) {
      return res
        .status(400)
        .json({ message: "Atleast maintain minimum balance 30 usd" });
    }

    if (stakeType === "flexible" && duration < 0) {
      return res
        .status(404)
        .json({ message: "flexible stake available only 24 hours" });
    }
...
router.get(
  "/radeemAmount/:stakeId",
  authenticate,
  authorize("user"),
  async (req, res) => {
    const { stakeId } = req.params;
    try {
      const radeem = await Staking.findOne(
        { _id: stakeId },
        { _id: 0, amount: 1 }
      );
...
router.post(
  ...
  async (req, res) => {
    ...
    const daysPassed = Math.floor(
      (now - new Date(stake.stakeDate)) / (1000 * 60 * 60 * 24)
    );
